...
navOptions.forEach((option) => {
  option.addEventListener("click", () => {
    if (option.classList.contains("optionAccount")) {
      ...
      optionStat.classList.toggle("active");
      let dropdownContainer = option.querySelector(".dropdown-container");
      dropdownContainer.style.display = dropdownContainer.style.display === "block" ? "none" : "block";
...
let accountPages = ["adminAccountsAdviser.html"];

let matchedPage;
if (accountPages.some(page => {
  if (currentPage.includes(page)) {
    matchedPage = page;
    return true;
  }
})) {
  let dropdownContainer = optionAccount.querySelector(".dropdown-container");
